...
extension Person
{
  ...
  {
    return (likedJobsMain + likedJobsSecondary).reduce(false) { $0 ? $0 : jobs.contains($1) }
  }
}
...
func getStringsFromList (list: [String], optionalPreferences: [String]?) -> [String]
{
  ...
  {
    return list.filter { preferences.contains($0) }
  }
  else
  {
    return list
...
func placeNameForPerson_structured (person: Person, desk: DeskWithJobs) -> String
{
  ...
  if person.childrenCount == 0
  {
    if person.isYoung
    {
      ...
      let l2 = getStringsFromList(desk.availableJobs, optionalPreferences: l1)
      if l2.count == 0
      {
        if desk.availableJobs.filter({ person.dislikedJobs.contains($0) == false }).count > 0
        {
